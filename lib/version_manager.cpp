...
public:
    ...
    ~VersionManager();
    bool init_version(treeNode *p, treeNode *vp) override;
    ...
    bool get_version_pointer(unsigned long long id, treeNode *&p) override;
    bool get_latest_version(unsigned long long &id) override;
    bool get_version_log(std::vector<std::pair<unsigned long long, versionNode>> &version_log) override;
    bool empty() override;
};
